...
from utils.dataset import VideoJsonDataset, SingleVideoDataset, ImageDataset, VideoFolderDataset, CachedDataset
...
def get_train_dataset(dataset_types, train_data, tokenizer):
    ...
    for DataSet in [VideoJsonDataset, SingleVideoDataset, ImageDataset, VideoFolderDataset]:
        ...
        print(DataSet.__getname__())
        for dataset in dataset_types:
            if dataset == DataSet.__getname__():
                print('dataset type',DataSet.__getname__())
...
def log_visuals_table(text, predicted):
    try:
        # 🐝 Create a wandb Table to log text, video predictions, labels and accuracy
        table = wandb.Table(columns=["text", "video"])
        # for text, pred in zip(text, predicted.to("cpu")):
        #     table.add_data(text, wandb.Video(pred.numpy()*255, fps=8))
        wandb.log({"predictions_table": table}, commit=False)
    except Exception as e:
        print("Predictions encountered an error: ", e)
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if wandb.run is not None: # 🐝 Close your wandb run 
        wandb.init(
            job_type="train-job",
            project='t2v_stablediffusion'
        )
                    ...
                    if accelerator.is_main_process:
                        ...
                        with accelerator.autocast():
                            ...
                            if wandb.run is not None:
                                log_visuals_table([prompt], [video_frames])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/my_config_hq.yaml")
